...
public class RestrictAttribute : AuthorizeAttribute
{
    public string Value
    {
        get
        {
            return Policy![AuthorizePolicy.POLICY_PREFIX.Length..] ?? string.Empty;
        }
        set
        {
            Policy = $"{AuthorizePolicy.POLICY_PREFIX}{value}";
        }
    }
    ...
    public RestrictAttribute(string? roles = null, string? claims = null)
    {
        ...
        Value = true switch
        {
            bool when
                ...
                && !string.IsNullOrWhiteSpace(claims) => roles + dash + claims,
